...
class Postfeed extends Component {
  ...
  componentDidMount() {
    if (this.props.userId) {
      PostAPI.getPostId(this.props.userId)
        .then(data => {
          this.setState({
            posts: data.data || []
          });
      ...
      UserAPI.getUser().then(data => {
        PostAPI.getPostId(data.data._id).then(data => {
          this.setState({
            posts: data.data || []
          });
        });
        ...
        {this.state.posts.map(post => {
          ...
          return (
            <Post
              ...
              author={
                post.ogAuthor ? post.ogAuthor.fullName : post.author.fullName
              }
              post={post.post}
              type={post.type}
